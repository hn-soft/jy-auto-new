...
export default class ProfessionalTextMerger extends React.Component<PropsType, StateType> {
  ...
  handleSelectProject = async (pInfo: PInfoType) => {
    // @ts-ignore
    const res = await window.electronAPI.professionalMergeText({
    ...
    if (res.status === "success") {
      this.setState({
        isResultModalOpen: true,
        resultModalText: `成功了。成功向左紧靠了${res.data}段字幕。`,
        showUndoBtn: true,
      });
    }
  };
  ...
  renderConfirmContentComp = (pInfo: PInfoType) => {
    return (
      <div>
        <p>{`您是否确定要让草稿 ${pInfo.draft_name} 里的字幕向左靠拢，使其紧密相靠？`}</p>
        <p>{"注意："}</p>
        <p>{"1. 建议在剪映中复制草稿作为备份。"}</p>
        <p>{"2. 请确认剪映软件中该草稿处于关闭状态，否则会彼此相互干扰。"}</p>
        <p>{"3. 填充操作会保持字幕片段的时长不变，右边的字幕的位置会向左移动，使得字幕之间紧靠无间隙。如果不满意，可点右下角撤销按钮。"}</p>
        <p>{"4. 如果你有多条字幕轨道且发现向左紧靠的不是你想要的字幕轨道，你可以点击不想要的轨道左侧的眼睛按钮隐藏此轨道，再重试。对于隐藏的轨道，本软件会忽略它。"}</p>
      </div>
    );
  };
  ...
  handleUndo = async () => {
    // @ts-ignore
    const res = await window.electronAPI.undoProfessionalMergeText();
    if (res.status === "error") {
      this.setState({
        isResultModalOpen: true,
        resultModalText: `失败了。原因: ${res.data}`,
  ...
  renderTrialIndication = () => {
    ...
    return (
      <div className="align-master-trial-container">
        ...
        <p>{`试用版限制只能让不超过10段字幕向左紧靠，正式版没有此上限。`}</p>
        <p>
    ...
    return (
      <div>
        ...
        <SelectProject
          ...
          headerFinePrint={"请点击你想要让字幕向左紧靠的草稿"}
        ...
        {this.state.isExplanationModalOpen ? (
          ...
          >
            <div style={{ height: 400, overflowY: "scroll" }}>
              <p style={{ fontSize: 18 }}>
                <strong>字幕向左紧靠</strong>的功能解释
              </p>
              <p>如图：</p>
              <p>
                <img
                  style={{ width: "100%" }}
                  alt="text-merger-demo"
                  src={"./illustration/textmergerdemo.png"}
                />
              </p>
              <p>
                {
                  "字幕片段本身的长度不变，移动其位置，让它们从左到右紧密排列。最左端的字幕会移动到从00:00开始。"
                }
              </p>
              ...
              <p>
                {
                  '那么你对齐图片或视频与字幕后再打开剪映会发现对不齐，这是因为主轨的素材都会向左紧靠，所以打开的一瞬间就打破了对齐关系，并不是本软件的Bug。"字幕向左紧靠"功能正是为了解决这一问题存在，但即便有此功能，还是建议你关闭"主轨磁吸"。'
                }
              </p>
              <p>
                {
                  '字幕向左紧靠操作会移动字幕的位置，字幕相对顺序不变。如果你想总时间保持不变，可以接受其字幕的时长延长，请你选择左侧的"填补字幕间隙"功能。'
                }
              </p>
            </div>
          </Modal>
