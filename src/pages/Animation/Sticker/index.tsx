...
export default class Filter extends React.Component<PropsType, StateType> {
  ...
  handleSelectTargetProject = async (pInfo: PInfoType) => {
    ...
    // @ts-ignore
    const res = await window.electronAPI.addStickers({
    ...
    if (res.status === "success") {
      this.setState({
        isResultModalOpen: true,
        resultModalText: `成功为${res.data.vSegCount}个视频片段添加了${res.data.layerCount}层贴纸，共${res.data.addedCount}个贴纸片段。为您节约了宝贵的时间！`,
        showUndoBtn: true,
      });
    }
  };
  ...
  renderConfirmContentComp = (pInfo: PInfoType) => {
    return (
      <div>
        <p>{`您是否确定要为 ${pInfo.draft_name} 批量添加贴纸?`}</p>
        <p>{`您设置的是${ this.state.randomMode === RANDOM_MODE.ORDER ? "顺序模式" : "纯随机模式" }。`}</p>
        <p>{`${ this.state.isTooShortToAdd ?  `时长短于${this.state.thresholdSegLen}s的片段不会添加贴纸` : "主轨道每个片段都要添加贴纸" }。`}</p>
        <p>{"注意："}</p>
  ...
  handleUndo = async () => {
    // @ts-ignore
    const res = await window.electronAPI.undoAddStickers();
    if (res.status === "error") {
      this.setState({
        isResultModalOpen: true,
        resultModalText: `失败了。原因: ${res.data}`,
    ...
    return (
      <div>
        ...
        <SelectProject
          ...
          headerFinePrint={
            "请选择参考草稿（该草稿中的具体贴纸会被用于添加到下方的目标草稿中）"
          }
          skipConfirm
        ...
        <div className="align-master-setting-whole-wrapper">
          ...
          <Row className="option-row">
            ...
            >
              <Radio value={false}>
                <span className="setting-text-span">主轨道每个片段都要添加贴纸</span>
              </Radio>
              <Radio value={true}>
                <span className="setting-text-span">主轨道上太短的片段不添加贴纸，由我定义阈值</span>
              </Radio>
            </Radio.Group>
          </Row>
          <Row className="option-row">
            { 
                ...
                <InputNumber
                    addonBefore={"阈值时长（短于此时长的片段不添加贴纸）:"}
                    addonAfter={"s"}
                    min={0}
                    max={100}
                    style={{ width: "400px" }}
        ...
        <SelectProject
          ...
          headerFinePrint={"请选择目标草稿（贴纸将会添加到该草稿中）"}
          hideProjectSourceSelect={IS_EN}
        />
        ...
        {this.state.isExplanationModalOpen ? (
          ...
          >
            <div style={{ height: 400, overflowY: "scroll" }}>
              ...
              <p>
                {`该功能可以让你批量添加贴纸到目标草稿，目标草稿主轨道上的每个片段都会对应于一个贴纸。如果你想要有许多不同种类的贴纸，你可以丰富你的参考草稿。参考草稿中已有的贴纸是本功能的选取来源。在实际使用中，参考草稿一般是一个放满了贴纸而没有其他内容的草稿，它专门用于本功能提取具体贴纸，你可以把你喜欢的贴纸都放进去。请注意：参考草稿中的贴纸如果被放在多条轨道，那么目标草稿操作后就有多条贴纸轨道，每条贴纸轨道的具体贴纸来源于参考草稿各条贴纸轨道上的贴纸。如果你想要目标草稿叠放3条贴纸轨道，你需要在参考草稿中放置3条贴纸轨道。`}
              </p>
              <p>{"使用步骤："}</p>
              <p>{`1. 选择参考草稿。`}</p>
