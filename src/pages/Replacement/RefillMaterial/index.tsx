...
import { arrangementCount } from "../../../utils/mathUtils";
import {
  Modal,
  Button,
  Row,
...
export default class RefillMaterial extends React.Component<
  PropsType,
  StateType
> {
  modiTransHelper: {
  ...
  handleSelectParentFolder = async () => {
    ...
    if (res.status === "success") {
      ...
      await this.handleCheckMaterialToRefill({
        parentFolderDir: res.data,
  ...
  handleCheckMaterialToRefill = async (param: {
    parentFolderDir: string;
    sourceExtnames: { extname: string; count: number }[];
  }) => {
    // @ts-ignore
    const res = await window.electronAPI.checkMaterialToRefill(param);
    if (res.status === "error") {
      this.setState({
        isResultModalOpen: true,
        resultModalText: `失败了。原因: ${res.data}`,
    ...
    return (
      <>
        <div className="align-master-setting-whole-wrapper">
          ...
          <Row>
            <span className="align-master-setting-enabled-title">{`文件结构的要求：请你在电脑中创建一个文件夹，然后将所有新素材放在这个文件夹里。`}</span>
          </Row>
          <Row>
          ...
          <Row>
            <div style={{marginTop: 13 }}>
              <span className="align-master-setting-big-title">
                {`接下来，请你在下方点击 "选中文件夹" 按钮，告知本软件这个文件夹在哪里。`}
              </span>
            </div>
          </Row>
        </div>
        <div className="align-master-setting-whole-wrapper">
          ...
          <Row style={{ marginTop: 10 }}>
            ...
            this.state.resourceInfos.length === 0 ? (
              <Button
                type="default"
                onClick={() => {
                  // just for ts
                  ...
                  this.handleCheckMaterialToRefill({
          ...
          this.state.resourceInfos.length > 0 ? (
            <>
              <div style={{ marginTop: 13 }}>
                <span className="align-master-setting-enabled-title">{`文件夹内有：${this.state.resourceInfos.map(resourceInfo => `${resourceInfo.resourceCount}个${resourceInfo.extname}文件`).join(", ")}。`}</span>
              </div>
              <div style={{ marginTop: 13 }}>
    ...
    const refillModeChoices = (
      <>
        ...
        <Row className="option-row">
          <Radio.Group
            onChange={(e: RadioChangeEvent) => {
              const resourceInfos = this.state.resourceInfos;
              let minCount = 0;
              let newProjectMaxPossibleCount = 1000;
              if (e.target.value === REFILL_SELECT_MODE_SOLUTION.ORDER) {
                const ratios = resourceInfos.map(info => Math.floor(info.resourceCount / info.sourceCount));
                minCount = Math.min(...ratios);
                minCount = Math.min(minCount, 1000);
                newProjectMaxPossibleCount = minCount;
              } else if (e.target.value === REFILL_SELECT_MODE_SOLUTION.RANDOM) {
                const mathArrangements = resourceInfos.map(info => arrangementCount(info.resourceCount, info.sourceCount));
                minCount = Math.min(...mathArrangements);
                minCount = Math.min(minCount, 1000);
                newProjectMaxPossibleCount = minCount;
              }
              this.setState({
      ...
      <div className="align-master-setting-whole-wrapper">
        ...
        {this.isSettingsFinished() ? (
          <>
            ...
            <Row>
              {this.state.refillSelectModeSolution.length > 0 ? newProjectCountInput : null}
            </Row>
          </>
        ) : (
          <Row style={{ paddingTop: 13 }}>
  ...
  handleClickAutoRefillMaterial = async () => {
  ...
  handleClickExecuteRefill = async () => {
    if (this.state.sourcePInfo == null || this.state.preData == null) {
      return;
    }
    this.setState({
    ...
    // @ts-ignore
    const res = await window.electronAPI.refillMaterial({
      sourcePInfoPath: this.state.sourcePInfo.draft_json_file,
      replaceTypesStr: this.state.replaceTypesStr,
      resourceInfos: this.state.resourceInfos,
      parentFolderDir: this.state.parentFolderDir,
      ...
      <div className="align-master-setting-whole-wrapper">
        ...
        <Row style={{ marginTop: 10 }}>
          <Button
            ...
            onClick={() => {
              if (this.state.isSelectAutoMode) {
                this.handleClickAutoRefillMaterial();
              } else {
                this.handleClickExecuteRefill();
              }
            }}
          >
      ...
      <div style={{ marginBottom: 40 }}>
        ...
        {this.state.isExplanationModalOpen ? (
          ...
          >
            ...
            >
              <a
                href="https://www.bilibili.com/video/BV1it421h7mA/"
                target="_blank"
                rel="noopener noreferrer"
              >
                {`【视频教程（七个颜色片段填充三个白色视频片段位置）】`}
              </a>
              <p>{"该功能适合于混剪裂变替换素材，无论你的图片视频素材有一个，还是有多个，都可以用此功能。如果你有多个草稿区域需要替换但彼此之间不可以混用，比如你的视频有 片头.mp4, 主要内容.mp4, 片尾.mp4 三者，而你有三批新素材（片头素材，主要内容素材，片尾素材），那你用此方法，片头和主要内容和片尾就会混起来，这是不合适的，这种情况下你应该使用左侧的分区混剪裂变替换素材。"}</p>
              <p>{"使用步骤:"}</p>
              <p>{"1. 选择一个草稿作为参考草稿（以此草稿为依据进行替换，生成相似的新草稿）"}</p>
              <p>{"2. 选择替换片段类型。"}</p>
              <p>{"3. 将所有素材放入同一个文件夹中并在本软件里选中它（注意是单层结构即可）。"}</p>
              <p>{"4. 完成替换设置。"}</p>
              <p>{"5. 点击 立即执行 按钮。"}</p>
            </div>
          </Modal>
