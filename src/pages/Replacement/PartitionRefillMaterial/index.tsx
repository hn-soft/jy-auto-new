...
type StateType = {
  ...
  newProjectMaxPossibleCount: number;
...
export default class PartitionRefillMaterial extends React.Component<
  PropsType,
  StateType
> {
  modiTransHelper: {
  ...
  constructor(props: PropsType) {
    ...
    this.state = {
      ...
      newProjectCount: 0,
      newProjectMaxPossibleCount: 1000,
  ...
  handleUndo = async () => {
    // @ts-ignore
    const res = await window.electronAPI.undoRefillMaterial();
    if (res.status === "error") {
      this.setState({
  ...
  renderTrialIndication = () => {
    ...
    return (
      <div className="align-master-trial-container">
        ...
        <p>{`试用版限制只能批量替换3段及以下的片段，正式版可以替换每套草稿里的无数段图片和视频片段。`}</p>
        <p>{`试用版限制一次至多可以裂变出5套新草稿，正式版都限制一次至多可以裂变出1000套新草稿。`}</p>
        <p>
  ...
  handleSelectParentFolder = async () => {
    ...
    if (res.status === "success") {
      ...
      await this.handleCheckMaterialToPartitionRefill({
        parentFolderDir: res.data,
        basenameParts: this.state.preData.basenameParts,
      });
    }
  };

  handleCheckMaterialToPartitionRefill = async (param: {
    parentFolderDir: string;
    basenameParts: { prefix: string; suffix: string }[];
  }) => {
    // @ts-ignore
    const res = await window.electronAPI.checkMaterialToPartitionRefill(param);
    if (res.status === "error") {
      this.setState({
        isResultModalOpen: true,
        resultModalText: `失败了。原因: ${res.data}`,
    ...
    if (res.status === "success") {
      this.setState({
        partitionMaterialsInfos: res.data,
        targetPInfos: [],
        nonEmptyTargetPInfos: [],
      });
    }
  ...
  renderSourceAnalysis = () => {
    ...
    const ignoredWord =
      this.state.preData.eachCount.ignoredVideoTrackCount > 0
        ...
        : "";
    const basenameParts = this.state.preData.basenameParts;
    const extWordsPrefix = "具体从文件的角度说来，你的任务是替换该草稿里的：";
    let basenames: string[] = [];
    for (let i = 0; i < basenameParts.length; i++) {
      basenames.push(`${basenameParts[i].prefix}${basenameParts[i].suffix}`);
    }
    const basenamesWord = basenames.join("; ");
    const basenameWordsSentence = `${extWordsPrefix}${basenamesWord}. 请注意：如果时间线上有部分素材片段你不需要替换，你可以将其文件命名为下划线开头，比如 _123.mp4，对于下划线开头的素材，本软件会将其原封不动复制到新草稿，而不寻求替换。`;
    let details = [];
    const uniqBasenameParts = basenameParts.filter((item, pos) => {
      const combined = `${item.prefix}${item.suffix}`;
      return basenames.indexOf(combined) === pos;
    });
    for (let i = 0; i < uniqBasenameParts.length; i++) {
      const basenamePart = uniqBasenameParts[i];
      details.push(
        `${i + 1}. 请创建一个名为 ${
          basenamePart.prefix
        } 的文件夹，并把扩展名为 ${basenamePart.suffix} 的新素材文件放入其中.`
      );
    }
    const detailsWord = details.join("\n");
    return (
      <>
        <div className="align-master-setting-whole-wrapper">
          <Row>
          ...
          <Row>
            <span className="align-master-setting-enabled-title">
              {basenameWordsSentence}
            </span>
          </Row>
          <Row>
            <span className="align-master-setting-enabled-title">
              {
                "接下来，你需要指定存储替换素材的文件夹，使得本软件能够为你做批量素材替换。具体说来，你需要提供双层文件夹结构。请仔细看下面的细节："
              }
            </span>
          </Row>
          <Row>
          ...
          <Row>
            <span className="align-master-setting-enabled-title">{`然后，请你确保 ${uniqBasenameParts
              .map((item) => item.prefix)
              .join(",")} (共${
              uniqBasenameParts.length
            }个子文件夹) 都被放在同一个父文件夹中，比如都放在一个叫 sucai（也可以是其它名称） 的文件夹中。`}</span>
          </Row>
          <Row>
          ...
          <Row>
            <div style={{ marginTop: 13 }}>
              <span className="align-master-setting-big-title">
                {`接下来，请你在下方点击 "选中父文件夹" 按钮，告知本软件这个父文件夹（刚才所说的 sucai, 当然也可以是其它名称）在哪里。`}
              </span>
            </div>
          </Row>
        </div>
        <div className="align-master-setting-whole-wrapper">
          ...
          <Row style={{ marginTop: 10 }}>
            ...
            this.state.partitionMaterialsInfos.length === 0 ? (
              <Button
                type="default"
                onClick={() => {
                  // just for ts
                  ...
                  this.handleCheckMaterialToPartitionRefill({
          ...
          this.state.partitionMaterialsInfos.length === 0 ? (
            <div style={{ marginTop: 13 }}>
              <span className="align-master-setting-enabled-title">{`该文件夹不符合要求，请重选或者修改后点击"重新检查新素材数量"按钮。`}</span>
            </div>
          ) : null}
          {this.state.parentFolderDir.length > 0 &&
          this.state.partitionMaterialsInfos.length > 0 ? (
  ...
  renderSettings = () => {
      ...
      <>
        ...
        <Row className="option-row">
          ...
          >
            ...
            <Radio
              value={REPLACE_VIDEO_DECORATION_SOLUTION.YES_ALIGN_ATTRIBUTE_0}
            >
              <span className="setting-text-span">
                {"是，需要重新对齐（忽略锁定的轨道上的元素）"}
              </span>
            </Radio>
          </Radio.Group>
        </Row>
          ...
          <Radio.Group
            onChange={(e: RadioChangeEvent) => {
              const partitionMaterialsInfos =
                this.state.partitionMaterialsInfos;
              let minCount = 0;
              let newProjectMaxPossibleCount = 1000;
              const partitionMaterialCounts = partitionMaterialsInfos.map(
                (info) => info.resourceCount
              );
              if (e.target.value === REFILL_SELECT_MODE_SOLUTION.ORDER) {
                minCount = Math.min(...partitionMaterialCounts);
                minCount = Math.min(minCount, 1000);
                newProjectMaxPossibleCount = minCount;
              } else if (
                e.target.value === REFILL_SELECT_MODE_SOLUTION.RANDOM
              ) {
                const countMulti = partitionMaterialCounts.reduce(
                  (accum, current) => accum * current,
                  1
                );
                minCount = Math.min(countMulti, 1000);
                newProjectMaxPossibleCount = minCount;
              }
              this.setState({
                refillSelectModeSolution: e.target.value,
                newProjectCount: newProjectMaxPossibleCount,
                newProjectMaxPossibleCount: newProjectMaxPossibleCount,
              });
            }}
            value={this.state.refillSelectModeSolution}
          >
            <Radio value={REFILL_SELECT_MODE_SOLUTION.ORDER}>
              <span className="setting-text-span">
                {"顺序模式（不重复，可裂变数量由最少的新素材数量决定）"}
              </span>
            </Radio>
            <Radio value={REFILL_SELECT_MODE_SOLUTION.RANDOM}>
              <span className="setting-text-span">
                {
                  "随机裂变模式（打乱排序，但同一新素材不会重复出现在同个新草稿中）"
                }
              </span>
            </Radio>
          </Radio.Group>
        </Row>
        {this.state.refillSelectModeSolution ===
        REFILL_SELECT_MODE_SOLUTION.ORDER ? (
          <>
            <Row>
              <span className="align-master-setting-enabled-title">
    ...
    const newProjectCountInput = (
      <InputNumber
        addonBefore={"你想要裂变出多少新草稿？（可修改→）:"}
        addonAfter={`(最大可能值：${this.state.newProjectMaxPossibleCount}）`}
        min={1}
        max={this.state.newProjectMaxPossibleCount}
        step={1}
        value={this.state.newProjectCount}
        onChange={(value: number | null) => {
          this.setState({
            newProjectCount: typeof value === "number" ? value : 0,
          });
        }}
      />
    );
        ...
        {this.isSettingsFinished() ? (
          <>
            <Row style={{ paddingTop: 13 }}>
              <span className="align-master-setting-big-title">
                {`所有设置已完成。接下来，你可以修改下方你想要的新草稿数量，然后在剪映中手动创建对应数量的新的空白草稿，然后`}
  ...
  handleClickFinishCreateProject = async () => {
    ...
    const res = await window.electronAPI.checkNewProject({
      requiredCount: this.state.newProjectCount,
  ...
  handleClickWantAutoRefillMaterial = () => {
    this.setState({
      isSelectAutoMode: true,
    });
  };

  handleClickAutoPartitionRefillMaterial = async () => {
    ...
    // @ts-ignore
    const res = await window.electronAPI.autoPartitionRefillMaterial({
      sourcePInfoPath: this.state.sourcePInfo.draft_json_file,
      replaceTypesStr: this.state.replaceTypesStr,
      partitionMaterialsInfos: this.state.partitionMaterialsInfos,
      parentFolderDir: this.state.parentFolderDir,
      ...
      refillSelectModeSolution: this.state.refillSelectModeSolution,
      newProjectCount: this.state.newProjectCount,
  ...
  renderFinishCreateProject = () => {
    ...
    const renderOtherNote = () => {
      ...
      if (
        this.state.targetPInfos.length === 0 &&
        !this.state.isSelectAutoMode
      ) {
        return null;
      }
      if (
    ...
    return (
      <div className="align-master-setting-whole-wrapper">
        ...
        <Row style={{ marginTop: 10 }}>
          <Button type="primary" onClick={this.handleClickFinishCreateProject}>
            {`我已手动新建${this.state.newProjectCount}个草稿，并关闭了草稿窗口，我只需要批量替换草稿`}
          </Button>
        </Row>
        <Row style={{ marginTop: 10 }}>
          <Button
            type="primary"
            danger
            onClick={this.handleClickWantAutoRefillMaterial}
          >
            {`让软件为我自动化新建${this.state.newProjectCount}个草稿，然后批量替换，且自动导出全部草稿`}
          </Button>
        </Row>
  ...
  handleClickExecutePartitionRefill = async () => {
    if (this.state.sourcePInfo == null || this.state.preData == null) {
      return;
    }
    this.setState({
    ...
    // @ts-ignore
    const res = await window.electronAPI.partitionRefillMaterial({
      sourcePInfoPath: this.state.sourcePInfo.draft_json_file,
      replaceTypesStr: this.state.replaceTypesStr,
      partitionMaterialsInfos: this.state.partitionMaterialsInfos,
      parentFolderDir: this.state.parentFolderDir,
      ...
      refillSelectModeSolution: this.state.refillSelectModeSolution,
      newProjectCount: this.state.newProjectCount,
      targetPInfoPaths: this.state.targetPInfos.map(
        (item) => item.draft_json_file
      ),
      modifier: this.state.modifier,
    });
    if (res.status === "error") {
      ...
      <div className="align-master-setting-whole-wrapper">
        ...
        <Row style={{ marginTop: 10 }}>
          <Button
            ...
            onClick={() => {
              if (this.state.isSelectAutoMode) {
                this.handleClickAutoPartitionRefillMaterial();
              } else {
                this.handleClickExecutePartitionRefill();
              }
            }}
          >
            {`立即执行${
              this.state.isSelectAutoMode
                ? "(点击后请立刻聚焦剪映窗口，放在屏幕正中央)"
                : ""
            }`}
          </Button>
        </Row>
      ...
      <div style={{ marginBottom: 40 }}>
        ...
        <div style={{ float: "right", marginBottom: 15 }}>
          {this.state.showUndoBtn ? (
            <Button onClick={this.handleClickUndo}>{"撤销刚才的操作"}</Button>
          ) : null}
        </div>
        ...
        {this.state.isExplanationModalOpen ? (
          ...
          >
            <div style={{ height: 400, overflowY: "scroll" }}>
              <p>
                {
                  "如果你有多个草稿区域需要替换但彼此之间不可以混用，比如你的视频有 片头.mp4, 主要内容.mp4, 片尾.mp4 三者，那你就需要此功能了。假设你有三批新素材（片头素材，主要内容素材，片尾素材），用了此方法，片头和主要内容和片尾就不会混起来，而是各自取代各自的片段，这就是 分区 二字的意思。如果你误用了左侧的 混剪裂变替换素材（无分区），那么片头的内容就可能跑到片尾去。"
                }
              </p>
              <p>{"使用步骤:"}</p>
              <p>
                {
                  "1. 选择一个草稿作为参考草稿（以此草稿为依据进行替换，生成相似的新草稿）"
                }
              </p>
              <p>{"2. 选择替换片段类型。"}</p>
              <p>
                {
                  "3. 将各自区域的素材放入各自的文件夹，并且把这些文件夹都放在一个总文件夹，比如命名为 sucai 并在本软件里选中它（注意是双层文件夹结构）。"
                }
              </p>
              <p>{"4. 完成替换设置。"}</p>
              <p>{"5. 点击 立即执行 按钮。"}</p>
