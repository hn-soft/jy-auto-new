...
import type { ProfessionalMergeTextParamType } from "../../utils/types";

export const handleProfessionalMergeAudio = async (
  _event: any,
  ...
  try {
    ...
    const ags = audioTrack.segments;
    if (ags.length === 0) {
      return { status: "error", data: "weird, 音频轨道上没有任何音频片段" };
    }
    if (activationStatus.status === "trial" && ags.length > 10) {
      return {
        status: "error",
        data: `有${ags.length}段音频，超过了试用版不大于10段的限制（正式版无此上限限制）。请你删除部分音频片段，关闭剪映草稿后，再回本软件重试。试用满意的话，恳请您点击左侧的"激活软件"激活正式版~`,
      };
    }
    ags[0].target_timerange.duration =
      ags[0].target_timerange.start + ags[0].target_timerange.duration;
    ags[0].target_timerange.start = 0;
    for (let i = 1; i < ags.length; i++) {
      const seg1 = ags[i - 1];
      const seg2 = ags[i];
      const seg1End =
        seg1.target_timerange.start + seg1.target_timerange.duration;
      seg2.target_timerange.start = seg1End;
    }
    ...
    return { status: "success", data: `${ags.length}` };
  } catch (e) {
    // @ts-ignore
