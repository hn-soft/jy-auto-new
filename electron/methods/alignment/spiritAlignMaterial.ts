...
import type { SpiritAlignMaterialParamType } from "../../utils/types";

export const handleSpiritAlignMaterial = async (
  _event: any,
  ...
  try {
    ...
    if (activationStatus.status === "trial" && textTrack.segments.length > 30) {
      return {
        status: "error",
        data: `发现草稿里共有${textTrack.segments.length}段字幕需要对齐，超过了试用版不大于30段的限制（正式版无此上限限制）。请你留下不超过30组需要对齐的语音和字幕，将超出数量的删除，关闭剪映草稿后，再回本软件重试。请注意删除后留下的字幕和音频数应该相同，不要一不小心误删到数量不匹配。如果您试用满意的话，恳请点击左侧的"激活软件"激活正式版~正式版可以匹配无限多段。`,
      };
    }
    const failedMatchingTextContents = [];
    for (let i = 0; i < textTrack.segments.length; i++) {
      ...
      const matchAIAudioRes = matchAIAudio(
        ...
        combinedAudioSegs,
        jyConfig,
        param.lowestSpeedChoice,
        param.lowestSpeedAllowed
      );
      ...
      failedMatchingTextContents.push(extractSegPureContent(textSeg, jyConfig));
    }
    if (param.needMergeTracks) {
    ...
    return {
      status: "success",
      data: JSON.stringify({
        textCount: textTrack.segments.length,
        audioCount: combinedAudioSegs.length,
        successTextCount:
          textTrack.segments.length - failedMatchingTextContents.length,
        failedTextCount: failedMatchingTextContents.length,
        failedTextContents: failedMatchingTextContents,
      }),
    };
  } catch (e) {
    // @ts-ignore
...
const speedSingle = (
  ...
  let constrainedMaxDuration = textSeg.target_timerange.duration;
  if (lowestSpeedChoice === "min-1") {
    constrainedMaxDuration = audioSeg.source_timerange.duration;
  }
  if (lowestSpeedChoice === "min-user-defined") {
    constrainedMaxDuration = audioSeg.source_timerange.duration / lowestSpeedAllowed;
  }
  audioSeg.target_timerange.duration = Math.min(constrainedMaxDuration, textSeg.target_timerange.duration);
  setSpeedPart(
    audioSeg,
    jyConfig,
    audioSeg.source_timerange.duration / audioSeg.target_timerange.duration
  );
};
