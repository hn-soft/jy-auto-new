...
import type { ReplaceMaterialParamType,TimerangeType,AutoReplaceMaterialParamType,PInfoType } from "../../utils/types";
...
export const handleReplaceMaterial = async (
  _event: any,
  ...
  try {
    ...
    if (activationStatus.status === "trial" && childFolders.length > 5) {
      return {
        status: "error",
        data: `试用版限制一次性只能生成至多5套新草稿，而当前你请求的是生成${childFolders.length}套新草稿。建议你减少所需新草稿数量再重试，你也可以购买正式版，正式版单次限制为1000个新草稿。`,
      };
    }
    const resultStrs = [];
    const sourceJyConfig = JSONbig.parse(sourceJsonString);
    const taskCount = childFolders.length;
    for (let i = 0; i < taskCount; i++) {
      progressHandler.updateProgressInfo({
        fraction: i / taskCount,
        indication: `当前进度`,
      });
      ...
      const materialChildDir = path.resolve(materialDir, childFolders[i]);
      if (fs.existsSync(materialChildDir)) {
        fs.rmSync(materialChildDir, { recursive: true, force: true });
      }
      fs.cpSync(
        path.resolve(parentFolderDir, childFolders[i]),
        materialChildDir,
        { recursive: true }
      );
      ...
      const resRMV = await handleReplaceMaterialsVideo(
        i,
        param,
        targetJyConfig,
        materialChildDir,
...
// 对于单一目标json文件的修改操作，有i个子文件夹，就会调用i次此函数（i是第一个参数），执行结果是同位替换 targetJyConfig.
// 子文件夹里的信息在sourceExtnameInfos的第i个里，里面有各文件名，已经按字母序排列好。
const handleReplaceMaterialsVideo = async (
  i: number,
  param: ReplaceMaterialParamType,
  targetJyConfig: any,
  materialChildDir: string,
  replaceTypes: string[]
) => {
  const {
    sourceExtnameInfos,
  ...
  const sourceExtnamesForOneChildAndChildFolder = sourceExtnameInfos[i];
  const extnameExtraInfos =
    sourceExtnamesForOneChildAndChildFolder.sourceExtnamesForOneChild.map(
      (item) => {
  ...
  for (let i = 0; i < videoTracks.length; i++) {
    ...
    for (let j = 0; j < track.segments.length; j++) {
      ...
      if (
        ...
        // @ts-ignore
        const extnameExtraInfo = extnameExtraInfos.find(
          (item) => item.extname === extname
        );
        if (extnameExtraInfo === undefined) {
          return {
            status: "error",
            data: `Cannot find info for extname: ${extname}, maybe this is newly inserted to the source project by you.`,
          };
        }
        if (extnameExtraInfo.count - extnameExtraInfo.current === 0) {
          return {
            status: "error",
            data: `file count equals current which is wrong, loop will exceeds last. file will not be enough.`,
          };
        }
        const toReplaceGrandchild =
          extnameExtraInfo.grandchilds[extnameExtraInfo.current];
        ...
        if (isPhotoMaterial(candidateMaterial)) {
          ...
          if (
            aspectRatioSolution === REPLACE_ASPECT_RATIO_SOLUTION.RAISE_ERROR
          ) {
            const oldAspectRatio =
              candidateMaterial.width / candidateMaterial.height;
            const newAspectRatio = imageDim.width / imageDim.height;
            if (Math.abs(newAspectRatio - oldAspectRatio) > 0.05) {
              return {
                status: "error",
                data: `参考草稿的图片素材"${
                  candidateMaterial.material_name
                }"的宽高比是${oldAspectRatio.toFixed(
                  3
                )}，而子文件夹"${childFolder}"中对位的图片素材"${toReplaceGrandchild}"的宽高比是${newAspectRatio.toFixed(
                  3
                )}。这两者并不相等，由于你设置了"提示框报错，并终止流程"，本次操作以失败结束，没有任何一个新草稿生成。你可以考虑调整新素材的宽和高，或在设置中选择"依然做替换"，然后重试。`,
              };
            }
          }
        ...
        if (isVideoMaterial(candidateMaterial)) {
          ...
          if (
            ...
            const oldAspectRatio =
              candidateMaterial.width / candidateMaterial.height;
            const newAspectRatio = videoDim.width / videoDim.height;
            if (Math.abs(newAspectRatio - oldAspectRatio) > 0.05) {
              return {
                status: "error",
                data: `参考草稿的视频素材"${
                  ...
                  3
                )}，而子文件夹"${childFolder}"中对位的视频素材"${toReplaceGrandchild}"的宽高比是${newAspectRatio.toFixed(
                  3
                )}。这两者并不相等，由于你设置了"提示框报错，并终止流程"，本次操作以失败结束，没有任何一个新草稿生成。你可以考虑调整新素材的宽和高，或在设置中选择"依然做替换"，然后重试。`,
              };
            }
        ...
        candidateMaterial.material_name = toReplaceGrandchild;
        candidateMaterial.path = newMaterialPath;
        extnameExtraInfo.current++;
      }
    }
...
const areStickToRightEdge = (
  ...
  const aEnd = timerangeA.start + timerangeA.duration;
  const bEnd = timerangeB.start + timerangeB.duration;
  return Math.abs(aEnd - bEnd) < 200000; // 0.2s内
};
...
export const handleAutoReplaceMaterial = async (
  _event: any,
  ...
  try {
    ...
    for (let i = 0; i < taskCount; i++) {
      ...
      if (param.deleteAfterComplete) {
        fs.rmSync(path.resolve(parentFolderDir, childFolders[i]), {
          recursive: true,
          force: true,
        });
      }
      ...
      await delay(1000 + 1000 * chosenLatencyFactor);
      while (
        store.get(STORE_KEY.IS_3_EXIT_WINDOW) &&
        !isExitProjectDetected &&
        exitRetry < 3
      ) {
        isWinEnv() ? rb.keyTap("f4", ["alt"]) : rb.keyTap("w", ["command"]);
