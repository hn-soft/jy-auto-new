...
import { getResourceCombinations } from "../../utils/mathUtils";
...
import type { AutoPartitionRefillMaterialParamType } from "../../utils/types";

import type { PartitionRefillMaterialParamType } from "../../utils/types";

import type { PInfoType } from "../../utils/types";
...
export const handlePartitionRefillMaterial = async (
  _event: any,
  ...
  try {
    ...
    let elecIdxListList: number[][] = [];
    if (refillSelectModeSolution === REFILL_SELECT_MODE_SOLUTION.RANDOM) {
      const resourceCountList = partitionMaterialsInfos.map(
        (info) => info.resourceCount
      );
      const targetCombList = getResourceCombinations(
        resourceCountList,
        newProjectCount
      );
      if (targetCombList.length < newProjectCount) {
        return {
          status: "error",
          data: `最多只能裂变${targetCombList.length}个，小于所需新草稿数量${newProjectCount}`,
        };
      }
      elecIdxListList = targetCombList;
    } else if (refillSelectModeSolution === REFILL_SELECT_MODE_SOLUTION.ORDER) {
      const orderedElecListList: number[][] = [];
      const len = partitionMaterialsInfos.length;
      for (let j = 0; j < newProjectCount; j++) {
        const li = Array(len).fill(j);
        orderedElecListList.push(li);
      }
      // 结果是[[全0],[全1],[全2]...] 如何保障不会太多，答案是由前端保证，不过兜底是会报错。
      elecIdxListList = orderedElecListList;
    } else {
      return {
        status: "error",
    ...
    for (let i = 0; i < newProjectCount; i++) {
      ...
      const elecIdxList = elecIdxListList[i];
      ...
      for (let j = 0; j < partitionMaterialsInfos.length; j++) {
        const partitionMaterialInfo = partitionMaterialsInfos[j];
        const elecIdx = elecIdxList[j];
        const elecResource = partitionMaterialInfo.resourceNames[elecIdx];
        fs.cpSync(
          path.resolve(
            parentFolderDir,
            `${partitionMaterialInfo.prefix}`,
            elecResource
          ),
          path.resolve(
            materialDir,
      ...
      const resRMV = await handlePartitionRefillMaterialsVideo(
        param,
        elecIdxList,
        materialDir,
        targetJyConfig,
...
// elecList记录的是选到了partitionMaterialsInfos里的哪一个，数组长度等于partitionMaterialsInfos的数组长度，一一对应。
// 比如partitionMaterialsInfos包含了 季节 和 月份。然后electIdxList = [1,2];那么代表此新草稿选中的是夏天和三月。
const handlePartitionRefillMaterialsVideo = async (
  param: PartitionRefillMaterialParamType,
  elecIdxList: number[],
  ...
  if (elecIdxList.length !== partitionMaterialsInfos.length) {
    return {
      status: "error",
      data: "elecIdxList length not equal partitionMaterialsInfos length",
    };
  }
  for (let i = 0; i < videoTracks.length; i++) {
    ...
    for (let j = 0; j < track.segments.length; j++) {
      ...
      if (
        ...
        const infoIdx = partitionMaterialsInfos.findIndex(
          (item) => item.prefix === bp && item.suffix === extname
        );
        if (infoIdx === -1) {
          return {
            status: "error",
            data: `你在设置完选项后修改了剪映草稿使得时间线上找不到素材 ${bp}${extname}`,
          };
        }
        const partitionMaterialsInfo = partitionMaterialsInfos[infoIdx];
        const elecIdx = elecIdxList[infoIdx];
        const elecResourceName = partitionMaterialsInfo.resourceNames[elecIdx];
        if (elecResourceName === undefined) {
          return {
            status: "error",
            data: `素材文件夹里 ${bp} 的素材数量不够 ${
              elecIdx + 1
            } 个，你可能在设置选项之后移除了文件。请重启本软件再试一次。`,
          };
        }
        ...
        candidateMaterial.material_name = elecResourceName;
...
export const handleAutoPartitionRefillMaterial = async (
  _event: any,
  ...
  try {
    ...
    const isKeyboardPRVal = isKeyboardPR();
    console.log("isKeyboardPRVal", isKeyboardPRVal);
    await delay(5000);
    for (let i = 0; i < newProjectCount; i++) {
      progressHandler.updateProgressInfo({
        fraction: i / newProjectCount,
